...
//	private Audio aifEffect;
    ...
    */
    public void init() {
	    ...
	    // a sound
	    oggEffect = AudioLoader.getAudio("OGG", ResourceLoader.getResourceAsStream("testdata/LRBark.ogg"));
	    ...
	    oggStream = AudioLoader.getStreamingAudio("OGG", ResourceLoader.getResource("testdata/LRWeird.ogg"));
			
	    // can load mods (XM, MOD) using ibxm which is then played through OpenAL. MODs
	    // are always streamed based on the way IBXM works
	    modStream = AudioLoader.getStreamingAudio("MOD", ResourceLoader.getResource("testdata/r-dstrcl.xm"));
...
//	    aifEffect = AudioLoader.getAudio("AIF", ResourceLoader.getResourceAsStream("testdata/burp.aif"));
	    ...
	    // a sound
//	    wavEffect = AudioLoader.getAudio("WAV", ResourceLoader.getResourceAsStream("testdata/aliens_game_over2.wav"));
        } catch (IOException e) {
	    e.printStackTrace();
	}
    }
...
//					AudioImpl.getClass()
					try {
						...
						System.out.println(modStream.getClass().getName());
						for (Field field : AudioImpl.class.getDeclaredFields()) {
							System.out.println(field.getName());
						}
						
						Field field = MODSound.class.getDeclaredField("store");
						field.setAccessible(true);
						if (field.get(modStream) == null) {
							System.out.println("null");
						}
						SoundStore store = (SoundStore) field.get(modStream);
