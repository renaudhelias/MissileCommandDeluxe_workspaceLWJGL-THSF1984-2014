...
public class TempestMain implements IScreen {
	...
	private static final int SCREEN_DEPTH = 500*2*2; // rotate du terrain de 500x500 + skybox qui est dÈj‡ en *2
...
//		GL11.glHint(GL11.GL_PERSPECTIVE_CORRECTION_HINT, GL11.GL_NICEST);
		// init OpenGL here
		GL11.glLoadIdentity();
		// le Z est NEGATIF mais √ßa n'emp√®che qu'ici les param√®tres sont rentr√©s dans l'autre sens comme si POSITIF... ce sont des tailles en fait
...
//		GL11.glShadeModel(GL11.GL_SMOOTH);
		...
		// lumi√®re "global" qui rebondie pas normalement (murs)
		FloatBuffer ambiantLight = BufferUtils.createFloatBuffer(4);
		ambiantLight.put(new float []{0.8f,0.8f,0.8f,1.0f});
		ambiantLight.flip();
		// la vrai lumi√®re qui √©clair et rebondie logiquement (objets)
...
//		GL11.glLightModeli(GL11.GL_LIGHT_MODEL_TWO_SIDE, GL11.GL_TRUE);
		...
		GL11.glEnable(GL11.GL_DEPTH_TEST); // d√©j√† fait mais bon
...
//			System.exit(0);
		...
		if (level instanceof ScoreLevel) {
			ScoreLevel scoreLevel = (ScoreLevel)level;
			scoreLevel.youWin(winner);
		}
		if (score >0) {
			// score in
...
//			level.setArea(new Killapede());
		...
		// purge
		level.step();
		// ce hack ne marche pas de toute faÁon.
		font = null;
	}
...
//			
//			// c'est au level de g√©rer la projection des controles aussi 
