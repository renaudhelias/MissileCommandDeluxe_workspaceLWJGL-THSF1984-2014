...
/**
 * Le Missile est ï¿½ la fois un ICircle, et ï¿½ la fois un IProjectile.
 * Il a donc deux ï¿½tats, non cumulable
...
//	}
	...
	public void step() {
		...
		if (!death) {
			if (stateExplosing) {
				if (isRed) {
					if (explosingRayon>Tools.deduceRayon(level)*CalibrageProps.getInstance().getRatioExplosionRed()) {
						death = true;
					} else {
						explosingRayon+=1.0;
					}
				...
				// suivant la force de la flï¿½che
				...
				//isOutOfScreen
				// suivant taille ï¿½cran donnï¿½ lors du main
	...
	private void changeStateIntoExplosion2() {
		explosion.stop();
		explosion.playAsSoundEffect(1.0f, 0.1f, false);
	...
	public void kill(IMarcheOuCreve killer) {
		...
		if (!stateExplosing) {
			if (killer instanceof IMissile) {
				changeStateIntoExplosion();
			} else if (killer instanceof Maison3D) {
				changeStateIntoExplosion2();
			} else {
				death = true;
			}
		}
	...
	public boolean touch(ICircle3D canon) {
		...
		if (contact) {
			if (canon instanceof Maison3D) {
				...
				// déjà fait dans changeStateIntoExplosion
	//			explosion.playAsSoundEffect(1.0f, 0.1f, false);
			}
		}
		return contact;
